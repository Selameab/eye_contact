...
class EyeContact:
    ...
    # Constants of the controller (Chosen through trial and error)
    p_hor = 20
    p_ver = 40
    c_hor = 0.05
    c_ver = 0.0
    ...
    def callback(self, data):
        if len(data.faces) == 1:
            self.set_eye_degree_hor_relative((data.faces[0].point.y - self.c_hor) * self.p_hor)
            self.set_eye_degree_ver_relative(-(data.faces[0].point.z - self.c_ver) * self.p_ver)
